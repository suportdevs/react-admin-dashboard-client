...
const Sidebar = ({ inactive, setInactive }) => {
    ...
    return (
        <div className={`bg-slate-900 text-slate-300 ${inactive ? 'w-1/5' : 'w-20'} h-screen p-4 transition[weight] ease-in duration-300`}>
            ...
            <div className="mt-6">
                <div>
                    <div className="menu flex items-center justify-between p-3 rounded-sm cursor-pointer hover:bg-slate-800 hover:text-white" onClick={() => setDashboardExpend(!dashboardExpend)}>
                        ...
                        <svg className={`w-6 h-6 ${!inactive ? 'w-0 h-0 opacity-0 invisible' : ''} ${dashboardExpend ? 'rotate-180' : ''} transition[rotate] ease-in duration-500`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" >
                            <path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                        </svg>
                    </div>
                    <div className={`dropdown-menu flex flex-col items-center justify-between ${!dashboardExpend ? 'h-0 opacity-0 invisible' : ''} transition[height] ease-in duration-400`}>
                ...
                <div>
                    <div className="flex items-center justify-between p-3 rounded-sm cursor-pointer hover:bg-slate-800 hover:text-white" onClick={() => setContentExpend(!contentExpend)}>
                        ...
                        <svg className={`w-6 h-6 ${!inactive ? 'w-0 h-0 opacity-0 invisible' : ''} ${contentExpend ? 'rotate-180' : ''} transition[rotate] ease-in duration-500`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" >
                            <path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                        </svg>
                    </div>
                ...
                <div>
                    <div className="flex items-center justify-between p-3 rounded-sm cursor-pointer hover:bg-slate-800 hover:text-white" onClick={() => setWarehouseExpend(!warehouseExpend)}>
                        ...
                        <svg className={`w-6 h-6 ${!inactive ? 'w-0 h-0 opacity-0 invisible' : ''} ${warehouseExpend ? 'rotate-180' : ''} transition[rotate] ease-in duration-500`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" >
                            <path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                        </svg>
                    </div>
